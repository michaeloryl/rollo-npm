...
var functions = {
  ...
  '^getDefaultSpeed': function() { return state.defaultSpeed; },
  '^getHeading': function() { return state.heading; },
  '^getCommandCount': function() { return state.cmdCount; },
  '^getUnknownCommandCount': function() { return state.unknownCmdCount; }
};
...
function doUntil(params, cb) {
  ...
  async.doUntil(function (next) {
    console.log('  do until: loop');
    executeLines(lines, function (err) {
      console.log('  do until: end-loop');
      return next();
    });
  }, function () {
    return operands[op](a, b);
  }, function (err, res) {
    console.log("do until: end");
    return cb();
  });
}
